...
library(leaflet)
leaflet(strava) %>% addProviderTiles(providers$Esri.WorldStreetMap) %>%
  addPolylines()

##load NWB
nwb_light<- readOGR("db/01-06-2017/NWB-Light/nwb-light.shp")
...
proj4string(nwb_light)<- rd #definieer de projectie van shapefile
nwb_light_wgs<- spTransform(nwb_light,wgs) #transformeer naar wgs84


leaflet() %>% addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  setView(lng = 4.63, lat = 52.015, zoom = 14) %>%
  addPolylines(data=strava,col="red") %>%
  addPolylines(data=nwb_light_wgs,weight = 10, opacity = 0.5,)
