...
x = pblapply( c(1:length(nwb_select@lines)), function(i){
  as.data.frame(spacing(pad = nwb_select@lines[[i]]@Lines[[1]]@coords  ,lengte= lengte))
})

for(i in 1:length(nwb_select@lines)){
  colnames(x[[i]]) = c('x','y')
  nwb_select@lines[[i]]@Lines[[1]]@coords = as.matrix(x[[i]])
}
...
for(i in 1:length(basemap_select@lines)){
   colnames(x[[i]]) = c('x','y')
  basemap_select@lines[[i]]@Lines[[1]]@coords = as.matrix(x[[i]])
}
