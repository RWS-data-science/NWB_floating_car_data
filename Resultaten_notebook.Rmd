...
distance_matrix = pblapply(1:length(NWB@lines), function(i){
  ...
  distances = lapply(1:nrow(NWB@lines[[i]]@Lines[[1]]@coords ), function(j){
     sqrt(  rowSums( (points_OSM - NWB@lines[[i]]@Lines[[1]]@coords[j,])^2 ) ) 
    
  })
  distances = unlist(distances)
  return( data.frame( 'ID' = NWB@lines[[i]]@ID  , 'max' =  max(distances), 'min' = min(distances) , 'mean' = mean(distances), 'median' =  median(distances)  ))
