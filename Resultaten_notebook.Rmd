...
basemap_select@data$twee_richting = 0

basemap_select@data$twee_richting[ basemap_select@data$segmentID %in% IDs_twee_richting  ]  = 1
...
nwb_select$is_nn50<- ifelse(nwb_select$WVK_ID %in% basemap_select$nearest_nwb_line_id[basemap_select$afstand_nwb_lijn<50],FALSE,TRUE )
...
nwb_select$vmax_OSM <- basemap_select$maxSpeedKPH[match(nwb_select$nn_OSM,basemap_select$segmentID)]
nwb_select$vOpt_OSM <- basemap_select$optSpeedKPH[match(nwb_select$nn_OSM,basemap_select$segmentID)]


##straatnamen
nwb_select$straatnaam_OSM <- basemap_select$name[match(nwb_select$nn_OSM,basemap_select$segmentID)]
...
wgs1<- spTransform(nwb_select,"+proj=longlat +ellps=WGS84 +datum=WGS84 +towgs84=0,0,0")
wgs2<- spTransform(gemeenten,"+proj=longlat +ellps=WGS84 +datum=WGS84 +towgs84=0,0,0")

leaflet() %>%  addProviderTiles(providers$CartoDB) %>%
 # addPolygons(data=wgs2,popup=~gemeentena) %>%
  addPolylines(data=wgs1[which(wgs1$string_dist >2),],col="red",popup=~paste("str = ",STT_NAAM,"<br>", "dist = ", max_afstand_osm)) 
...
nwb_select$dekking_dag<- dag_agg$perc_coverage[match(nwb_select$nn_OSM,dag_agg$SegmentID)]

nwb_select$v_mean_nacht<- nacht_agg$v_mean[match(nwb_select$nn_OSM,nacht_agg$SegmentID)]
nwb_select$dekking_nacht<- nacht_agg$perc_coverage[match(nwb_select$nn_OSM,nacht_agg$SegmentID)]
