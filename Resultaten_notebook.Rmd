...
distance_matrix_OSM_matching = pblapply(1:length(nwb_select_eq@lines), function(i){
  ...
  return( data.frame( 'WVK_ID' = as.numeric(as.character(nwb_select_eq@data$WVK_ID[i])), 'max' =  max(distances), 'min' = min(distances) , 'mean' = mean(distances), 'median' =  median(distances)  ))
...
rm(distance_matrix_OSM_matching)
