...
Blijkbaar komen in de OSM basemap veel dubbele wegsementen voor (exact dezelfde geometrie). Het heeft geen zin om deze subbel te checken, daarom verwijderen we ze hier.
```{r}
...
while(i <= length(basemap_select@lines)){
  ...
  if(! (basemap_select@lines[[i]]@ID %in% IDs)){
     basemap_select<- basemap_select[-i,] # = NULL
    #basemap_select@data[i,] = NULL
    i=i
  }else{
    i = i+1
    if (!i%%100){print(i)}
...
#   })
# 
#   minimum_distance_nwb = min(unlist( hausdorf_distances_to_OSM_lines))
#   label = which.min( unlist(hausdorf_distances_to_OSM_lines) )
# 
#   return(c(as.numeric(as.character(NWB$WVK_ID[i])),as.numeric(as.character(OSM$segmentID[label])), minimum_distance_nwb))
# })
...
dichtst_bijzijnde_OSM_junctie =  pblapply(c(1:nrow(NWB_juncties)), function(i){
 dist =  sqrt ( (OSM_juncties[,1] - NWB_juncties[i,1])^2   +   (OSM_juncties[,2] - NWB_juncties[i,2])^2 )
 buren = OSM_juncties[dist == min(dist),]
 x = data.frame('x' =  buren[1,1], 'lat' =  buren[1,2], 'dist' = min(dist))
...
colnames(juncties) = c('x_NWB', 'y_NWB', 'x_OSM', 'y_OSM', 'dist')


#vind de wegvak IDs erbij
#Maak dataframe van alle punten in het NWB plus ID

totaal = lapply(1:length(nwb_select@lines), function(i){
output = as.data.frame(nwb_select@lines[[i]]@Lines[[1]]@coords)
colnames(output) =c('x','y')
output$id = as.numeric(as.character(nwb_select@data$WVK_ID[i]))
  return(output)
})

totaal = rbindlist(totaal)

#zoek de IDs op van de juncties
juncties$ID = -1

for(i in 1:nrow(juncties)){
  z = totaal$ID[  juncties$x_NWB[i] == totaal$x & juncties$y_NWB[i] == totaal$y  ]
  juncties$ID[i] = z[1]
}
