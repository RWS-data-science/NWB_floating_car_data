...
distance_lijst_OSM = pblapply(c(1:length(basemap_select_eq@lines)), function(i){
  ...
  return(c(basemap_select_eq$segmentID[i],as.numeric(as.character(nwb_select_eq$WVK_ID[label])), minimum_distance_osm))
})

#create df with nearest neighbors

#voor ieder stukje NWB, max, min, mean en median distance naar dichtsbijzijnde OSM punten.
points_OSM = pblapply(c(1:length(basemap_select_eq@lines)), function(i){
  as.data.frame(basemap_select_eq@lines[[i]]@Lines[[1]]@coords)
})
...
distance_matrix_OSM_matching = pblapply(1:length(nwb_select_eq@lines), function(i){
  
  distances = lapply(1:nrow(nwb_select_eq@lines[[i]]@Lines[[1]]@coords ), function(j){
     min(sqrt(  ( (points_OSM$x - nwb_select_eq@lines[[i]]@Lines[[1]]@coords[j,1])^2 + (points_OSM$y - nwb_select_eq@lines[[i]]@Lines[[1]]@coords[j,2])^2) ) )
    
  })
  distances = unlist(distances)
  return( data.frame( 'ID' = as.numeric(as.character(nwb_select_eq@data$WVK_ID[i])), 'max' =  max(distances), 'min' = min(distances) , 'mean' = mean(distances), 'median' =  median(distances)  ))
  
})

distance_matrix_OSM_matching = rbindlist(distance_matrix_OSM_matching)
...
colnames(juncties) = c('x_NWB', 'y_NWB', 'id_nwb', 'x_osm', 'y_OSM', 'id_osm', 'dist')
...
##Vind afwijkende vormen doormiddeln van afstand punten NWB tot punten OSM
...
distance_matrix = pblapply(1:length(NWB@lines), function(i){
  ...
  distances = unlist(distances)
  return( data.frame( 'ID' = as.numeric(as.character(NWB@data$WVK_ID[i])), 'max' =  max(distances), 'min' = min(distances) , 'mean' = mean(distances), 'median' =  median(distances)  ))
...
python.load("omrekenen.py")
  ...
  juncties = lapply( 1:nrow(matrix_juncties), function(i){
   ...
   coords = data.frame('id' = matrix_juncties$id_nwb[i], 'dist' = matrix_juncties$dist )
...
m = addCircleMarkers(m, lat = juncties$lat_nwb , lng = juncties$lon_nwb, color ='blue', popup =  juncties$dist )
m = addCircleMarkers(m, lat = juncties$lat_osm , lng = juncties$lon_osm, color ='red', popup = juncties$dist )
