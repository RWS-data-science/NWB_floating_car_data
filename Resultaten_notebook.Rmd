...
x = pblapply( c(1:length(basemap_select@lines)), function(i){
  spacing(pad = basemap_select@lines[[i]]@Lines[[1]]@coords  ,lengte= lengte)
})

# for (i in 1:nrow(basemap_select)){
#   if(!any(is.na(x[[i]]))){ #alleen als er geen NA's zijn geproduceerd  @daan, code is aangepast zodat dit als het goed is niet meer gebeurt
#   basemap_select@lines[[i]]@Lines[[1]]@coords<- x[[i]]
#   }
# }
...
##MATCH lines door middel van meand distance
...
points_OSM = pblapply(c(1:length(OSM@lines)), function(i){
  OSM@lines[[i]]@Lines[[1]]@coords
})
points_OSM = do.call(rbind, points_OSM)
points_OSM = points_OSM[!duplicated(points_OSM),]

#loop door alle NWB lines heen en bereken voor iedere line de minimale afstand tot een punt in OSM

NWB_to_OSM_points = pblapply(c(1:length(NWB@lines)), function(i){
  
  min_distances = lapply(c(1:nrow(NWB@lines[[i]]@Lines[[1]]@coords)), function(j){
    distances = sqrt( (  NWB@lines[[i]]@Lines[[1]]@coords[j,1] - points_OSM[,1]  )^2  +  (  NWB@lines[[i]]@Lines[[1]]@coords[j,2] - points_OSM[,2]  )^2 )
    min(distances)
    
  })
  min_distances = unlist(min_distances)
  
  return( c(max(min_distances), mean(min_distances)))
})

NWB_to_OSM_points = do.call(rbind, NWB_to_OSM_points)
colnames(NWB_to_OSM_points) = c('max_distance', 'mean_distance')
```
>>>>>>> 71591fac6c77b5e0fdd1f276afeb316e7872b963
